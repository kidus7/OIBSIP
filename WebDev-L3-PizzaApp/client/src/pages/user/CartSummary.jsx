...
export default function CartSummary() {
  ...
  if (!cart || cart.length === 0) {
    return (
      <div className="min-h-screen bg-linear-to-b from-orange-50/50 via-white to-orange-50/30 pt-28 pb-20 flex items-center justify-center">
        <div className="max-w-md w-full mx-4 bg-white rounded-3xl p-8 shadow-2xl border border-slate-100 text-center">
          <div className="w-20 h-20 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl shadow-inner animate-pulse">
            🛒
          </div>
          ...
          <div className="space-y-3">
            ...
            <Link
              to="/dashboard"
              className="w-full block py-3.5 px-6 rounded-2xl font-bold text-sm bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all text-center"
            >
    ...
    <div className="min-h-screen bg-linear-to-b from-orange-50/50 via-white to-orange-50/30 pt-20 pb-32 sm:pb-20 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          ...
          <button
            onClick={clearCart}
            className="self-start md:self-auto px-4 py-2 rounded-xl text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 transition-colors border border-red-200 flex items-center gap-2 shadow-sm"
          >
            <span>🗑️</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-4">
            {cart.map((item, index) => {
              ...
              return (
                <div
                  key={itemId}
                  className="bg-white rounded-3xl p-5 sm:p-6 shadow-xl border border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 hover:shadow-2xl transition-shadow"
                >
                  <div className="flex items-start sm:items-center gap-4 w-full sm:w-auto">
                    {item.image ? (
                      ...
                      <div className="w-20 h-20 sm:w-24 sm:h-24 bg-linear-to-br from-orange-100 to-amber-100 rounded-2xl flex items-center justify-center text-3xl shadow-inner shrink-0">
                        🍕
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5 overflow-hidden">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase ${
                          isCustom ? 'bg-orange-100 text-orange-800' : 'bg-emerald-100 text-emerald-800'
                        }`}>
                      ...
                      {isCustom ? (
                        <div className="text-xs text-slate-600 space-y-0.5 pt-1">
                          <p><span className="font-bold text-slate-700">Crust:</span> {item.base}</p>
                          <p><span className="font-bold text-slate-700">Sauce:</span> {item.sauce}</p>
                          <p><span className="font-bold text-slate-700">Cheese:</span> {item.cheese}</p>
                          {item.veggies && item.veggies.length > 0 && (
                            <p><span className="font-bold text-slate-700">Veggies:</span> {item.veggies.join(', ')}</p>
                          )}
                        </div>
                      ) : (
                      ...
                      <div className="text-sm font-black text-orange-600 pt-1">
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-slate-100 gap-3">
                    <div className="flex items-center gap-3 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                      <button
                        ...
                        className="w-7 h-7 bg-white rounded-lg shadow-sm border border-slate-200 font-bold text-slate-700 hover:bg-orange-50 hover:text-orange-600 flex items-center justify-center transition-colors"
                      >
                        -
                      </button>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-28 bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-orange-100 space-y-6 mb-24 lg:mb-0">
            ...
            <div className="space-y-3 pt-2 hidden lg:block">
              ...
              <Link
                to="/dashboard"
                className="w-full block py-3.5 min-h-[48px] rounded-2xl font-bold text-sm bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all text-center"
              >
                ← Continue Shopping
              </Link>
            </div>

            <div className="bg-orange-50 rounded-2xl p-4 border border-orange-200 text-center hidden lg:block">
              <span className="text-xs font-bold text-orange-800 block">⚡ Blazing Fast 30-Min Delivery</span>
              <span className="text-[10px] text-slate-600 block mt-1">Guaranteed hot & fresh pizza delivered to your doorstep.</span>
            </div>
          </div>
